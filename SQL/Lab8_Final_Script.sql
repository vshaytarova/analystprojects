...
/*=====================================================================*/
/* Lab 7 – Database Warehouse (combination of Lab 5, 6 and 7)   */
/* Name: xxxxxxxxxxxxxx                                                */
...
--Use Master Database to be able to drop other databases. It is not possible to drop a database, which is in use.
USE master;  
--GO statement is used as a Batch separator in Sql Server
GO  
...
CREATE TABLE [Location] (
   LocationID	int identity(1,1)	NOT NULL, --Type the attribute name, value type and null restrictions, if required
   StreeAddress	nvarchar(100)	NOT NULL,
   City			nvarchar(50)	NOT NULL,
   Province		char(2)			NOT NULL,
   PostalCode	char(7)			NOT NULL,
   CONSTRAINT PK_LOCATION PRIMARY KEY (LocationID) --Specify the primary key
)
GO
...
CREATE TABLE Dealership (
   ...
   CONSTRAINT FK_DEAL_LOC FOREIGN KEY (LocationID) REFERENCES Location (LocationID) --Create a foreign key to refer to a primary key in other table.
)
GO
...
CREATE TABLE Person (
   ...
   CONSTRAINT CHK_TITLE	CHECK (Title='Mr' OR Title ='Ms')	--Add constraint to check if the value is either Mr or Ms. Others are not acceptable.
)
GO
/*==============================================================*/
...
CREATE TABLE SalaryEmployee (
   EmployeeID		int				NOT NULL,
   Salary			decimal(12,2)	NOT NULL DEFAULT 1000.00, --Add defaul value for an attribute
   ...
   CONSTRAINT CHK_SALARY	CHECK (Salary>=1000) --Add constraint on minimum amount of salary
)
GO
...
CREATE TABLE CommissionEmployee (
   ...
   CONSTRAINT CHK_COMMISSION	CHECK (Commission>=10) --Add constraint on minimum commission
)
GO
...
CREATE TABLE Vehicle (
   ...
   CONSTRAINT CHK_YEAR	CHECK (VehicleYear>=1800 AND VehicleYear <= YEAR(GETDATE())) --The year constraint is set to avoid error in entering data
...
CREATE TABLE [Order] (
   ...
   OrderDate	date			NOT NULL DEFAULT GetDate(), --Add the date by default as today, when the user enter the data
...
BEGIN TRY --Implements error handling for Transact-SQL. Add BEGIN TRY in the begging of the statement, and add CATCH at the end.
...
SELECT * FROM [Location] --Test the code above
...
SELECT * FROM Vehicle --Test the code above
...
SELECT * FROM Dealership --Test the code above
...
SELECT * FROM Employee --Test the code above
...
SELECT * FROM CommissionEmployee --TEST FOR CODES ABOVE
...
SELECT * FROM OrderItem --Test codes above
...
-- Create and Load FactSales Fact table
	SELECT DISTINCT 
		...
		D.DealershipSK,
		OI.FinalSalePrice,
		CAST(((Commission/100)*FinalSalePrice) AS decimal(12,2)) AS CommissionAmount
